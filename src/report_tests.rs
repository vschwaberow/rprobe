...
mod tests {
    ...
    fn create_test_entries() -> Vec<ReportEntry> {
        vec![
            ReportEntry {
                ...
                detections: vec![
                    "Apache Basic: Apache/2.4.41".to_string(),
                    "PHP Basic: PHP/7.4.3".to_string(),
                ],
            },
            ReportEntry {
            ...
            ReportEntry {
                ...
                status: "200".to_string(),
                detections: vec![
                    "Wordpress Basic: WordPress Detected".to_string(),
                    "PHP Basic: PHP/8.0.0".to_string(),
                ],
            },
        ]
    }
    ...
    fn test_generate_html_report() {
        ...
        assert!(
            content.contains("<span class=\"detection-tag\">Apache Basic: Apache/2.4.41</span>")
        );
    ...
    fn test_generate_html_report_status_classes() {
        ...
        let entries = vec![
            ...
            ReportEntry {
                url: "https://notfound.com".to_string(),
    ...
    fn test_generate_report_with_format() {
        ...
        let text_result =
            ReportGenerator::generate_report(&entries, &text_path, ReportFormat::Text);
        assert!(text_result.is_ok());
        assert!(fs::metadata(&text_path).is_ok());

        let json_result =
            ReportGenerator::generate_report(&entries, &json_path, ReportFormat::Json);
