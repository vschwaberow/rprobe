...
fn generate_safe_filename(output_path: &str) -> std::result::Result<String, Box<dyn Error>> {
    ...
    if let Some(parent) = path.parent() {
        ...
        let canonical_path = path.canonicalize().unwrap_or_else(|_| {
            if let Some(filename) = path.file_name() {
                canonical_parent.join(sanitize_filename(&filename.to_string_lossy()))
            } else {
...
impl ReportGenerator {
    pub fn generate_report(
        ...
        let safe_path = generate_safe_filename(output_path)
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?;

        match format {
            ReportFormat::Text => Self::generate_text_report(entries, &safe_path),
            ReportFormat::Json => Self::generate_json_report(entries, &safe_path),
        }
    }
    ...
    pub fn generate_csv_report(entries: &[ReportEntry], output_path: &str) -> Result<()> {
        ...
        for entry in entries {
            ...
            writeln!(
                file,
                "{},\"{}\",\"{}\"",
                entry.url, entry.status, escaped_detections
            )?;
        }
    ...
    pub fn generate_html_report(entries: &[ReportEntry], output_path: &str) -> Result<()> {
        ...
        writeln!(
            file,
            "      <tr><th>URL</th><th>Status</th><th>Detections</th></tr>"
        )?;
